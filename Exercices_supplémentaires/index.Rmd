...
Sa route le mène au labyrinthe des Luncredi, illustré à la diapositive suivante.
...
À vue d'oeil, le chemin à prendre est unique et est indiqué en blanc.
...
Déterminez le pseudo-code à envoyer à Rémi afin de lui permettre de se rendre à la fin du labyrinthe. On peut penser à des étapes dans le style suivant:
...
Dans le fichier [DeplacementRemi.R](./donnees/DeplacementRemi.R), la fonction permettant de déterminer l'emplacement (la coordonnée x et y du plan cartésien) après chaque déplacement de Rémi y est écrite. L'objet **section1** nous renvoie la matrice des positions empruntées par Rémi lors de ces déplacements dans la section 1.
...
Bien sûr, ce mouvement est valide dans tous les sens:
...
La section 4 n'est pas un mouvement horizontal ou vertical. Cependant, on peut exprimer le chemin à prendre comme l'hypoténuse du triangle rectangle suivant. 
...
L'objet **section1** était déjà défini dans le script R. Ainsi, il suffit de faire rouler cette portion de code afin d'obtenir les premières coordonnées empruntées.
...
```{r}
matfinal <- rbind(section1,section2,section3,section4) # rbind() pour combiner toutes les matrices ensembles:
```
...
En appliquant une boucle imbriquée (une boucle dans une autre boucle), construisez les matrices suivantes:

.pull-left[
Matrice 1:

<img src="assets/img/matrice1.PNG" height="150px"></img>
...
<img src="assets/img/matrice2.PNG" height="150px"></img>


</div>
]

Indice: Pour une même ligne (ou colonne), est-ce que les valeurs augmentent en changeant de colonne (ou ligne) et de combien est l'augmentation?
...
<img src="assets/img/matrice3.PNG" height="150px"></img>
...
<img src="assets/img/matrice4.PNG" height="150px"></img>


</div>
]

Pour la matrice 3, pensez à regarder la matrice 1. De plus, *?abs* pourrait vous être utile.
...
<img src="assets/img/matrice5.PNG" height="150px"></img>
...
<img src="assets/img/matrice6.PNG" height="150px"></img>


</div>
]

On augmente un peu la difficulté ici. 

Indice: Pour une même ligne (ou colonne), regardez l'augmentation entre les différentes valeurs. Par exemple, si on remarque que pour la ligne 1, on passe de 3 à 7, puis de 7 à 23, alors on remarque qu'on a une augmentation de $4= 4^1$, puis de $16 = 4^2$. On se doute alors que la ligne est définie comme $C + 4^{j}$ où $j$ pourrait représenter le numéro de la colonne et $C$ une constante quelconque.
...
- On remarque que pour une même ligne, il y a une augmentation de 1 lorsque la colonne change (lorsque $j$ augmente): $+j$.
- On remarque que pour une même colonne, il y a une diminution de 1 lorsque la ligne change (lorsque $i$ augmente): $-i$.
- La première coordonnée en ligne 0 et colonne 0 étant de 0, alors $(+1)+(-1)=0$ fonctionne et il n'est pas nécessaire d'ajouter une constante.
- On retrouve donc $j-i$ pour créer la matrice.

```{r}
mat1 <- matrix(NA, nrow = 5, ncol = 5)

for (i in 1:nrow(mat1)){
  for (j in 1:ncol(mat1)){
    mat1[i,j] <- j-i # Le j-i apparaît ici!
  }
}
```
...
- On remarque que pour une même ligne, il y a une augmentation de 2 lorsque la colonne change (lorsque $j$ augmente de 1): $+2*j$.
- On remarque que pour une même colonne, il y a une augmentation de 1 lorsque la ligne change (lorsque $i$ augmente de 1): $+i$.
- La première coordonnée en ligne 1 et colonne 1 étant de 4, alors $(+2*1)+(+1)=3$ ne fonctionne pas $(3 \ne 4)$ et il est nécessaire d'ajouter une constante $(+1)$ afin d'obtenir $(4 = 4)$.
- On retrouve donc $2*j+i+1$ pour créer la matrice.



```{r}
mat2 <- matrix(NA, nrow = 5, ncol = 5)

for (i in 1:nrow(mat2)){
  for (j in 1:ncol(mat2)){
    mat2[i,j] <- 2*j+i+1 # Le 2j + i + 1 apparaît ici!
  }
}
```
...
- On remarque que la matrice est 3 est la même que la matrice 1, mis-à-part les chiffres négatifs qui sont devenus positifs. De plus, on remarque que la fonction *abs(x)* permet d'obtenir la valeur positive de x. (Par exemple, *abs(3) = 3* et *abs(-4) = 4*)
- Deux choix sont donc possibles: appliquer la fonction *abs()* à **mat1**, ou d'appliquer *abs()* à **j-i**.

```{r}
mat3 <- matrix(NA, nrow = 5, ncol = 5)

for (i in 1:nrow(mat3)){
  for (j in 1:ncol(mat3)){
    mat3[i,j] <- abs(j-i) # Le abs(j-i) apparaît ici!
  }
}
#
# OU
#
mat3 <- abs(mat1) # abs(mat1) apparaît ici!
```
...
- On remarque que pour une même ligne, il n'y a pas d'augmentation lorsque la colonne change (lorsque $j$ augmente de 1): $0$.
- On remarque que pour une même colonne, il y a une augmentation de 1 lorsque la ligne change (lorsque $i$ augmente de 1): $+i$.
- La première coordonnée en ligne 1 et colonne 1 étant de 1, alors $(0)+(+1)=1$ fonctionne et il n'est pas nécessaire d'ajouter une constante.
- On retrouve donc $i$ pour créer la matrice.

```{r}
mat4 <- matrix(NA, nrow = 5, ncol = 5)

for (i in 1:nrow(mat4)){
  for (j in 1:ncol(mat4)){
    mat4[i,j] <- i
  }
}
```
...
- On remarque que pour une même ligne, l'augmentation n'est pas constante. Effectivement, on remarque qu'entre la colonne 1 et la colonne 2, une différence de (+2) est visible. Entre la colonne 2 et la colonne 3, une différence de (+4) est visible. En continuant, on observe une augmentation de (+8) et (+16).
- $(2;4;8;16) = (2^1;2^2;2^3;2^4)$
- Lorsque la colonne augmente de 1, l'exposant augmente de 1. On s'attend donc à apercevoir la valeur $2^j$.
- On remarque que pour une même colonne, il y a une diminution de 1 lorsque la ligne change (lorsque $i$ augmente): $-i$.
- Finalement, on remarque que pour la ligne 1 et colonne 1 (i=1, j=1), on doit obtenir la valeur 1. Hors, $(2^1 - 1) = 1$ d'où aucune constante supplémentaire n'est nécessaire.
- On retrouve donc $2^j-i$ pour créer la matrice.
...
```{r}
mat5 <- matrix(NA, nrow = 5, ncol = 5)

for (i in 1:nrow(mat5)){
  for (j in 1:ncol(mat5)){
    mat5[i,j] <- 2^j-i # Le 2^j-i apparaît ici!
  }
}
mat5
```
...
- On remarque que pour une même ligne, l'augmentation n'est pas constante. On augmente effectivement de (2;6;18;54).
- $2*3 = 6$, $6*3 = 18$, $18 * 3 = 54$. Il se cache donc un $3^j$ ou quelque chose d'assez semblable.
- On remarque que pour une même colonne, l'augmentation n'est pas constante. On augmente effectivement de (-1;-2;-4;-8).
- $-1*2 = -2$, $-2*2 = -4$, $-4*2 = -8$. Il se cache donc un $2^i$ ou quelque chose d'assez semblable.
- La valeur en ligne 1 et colonne 1 est 0. Hors, $3^1 - 2^1 = 1 \ne 0$. Nous devons donc soustraire 1.
- On s'attend à ce que $3^j - 2^i - 1$ soit le terme pour créer la matrice.
...
```{r}
mat6 <- matrix(NA, nrow = 5, ncol = 5)

for (i in 1:nrow(mat6)){
  for (j in 1:ncol(mat6)){
    mat6[i,j] <- 3^j-2^i-1 # Le 3^j - 2^i - 1 apparaît ici!
  }
}
mat6
```
...
Effectivement, la matrice obtenue n'est pas la même. Ainsi, il est fort à parier que ce n'est pas une constante que nous devrions ajouter, mais bien une modification dans les exposants.

On peut remarquer que $3^j - 2^i$, pour i=1 et j=1, est égale à 1. Comme on cherche à obtenir 0 comme réponse, on peut penser au fait que $3^0 = 1$ et que $2^0 = 1$, d'où $3^0 - 2^0 = 1 - 1 = 0$. 

Pour obtenir 0 comme exposant, il suffit de soustraire 1 à la ligne et à la colonne. 

On obtient alors $3^{j-1}$ et $2^{i-1}$, de sorte que $3^{j-1}-2^{i-1}$ est notre terme pour créer la matrice.
...
for (i in 1:nrow(mat6)){
  for (j in 1:ncol(mat6)){
    mat6[i,j] <- 3^(j-1)-2^(i-1) # Le 3^(j-1)-2^(i-1) apparaît ici!
  }
}
mat6
```
