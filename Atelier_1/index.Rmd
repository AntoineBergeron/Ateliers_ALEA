...
Après quelques essais infructueux, Thomas se ravise. Il préférerait indiquer à ses amis qu'il a été chanceux si 1 seul lancer fut nécessaire, qu'il n'est pas chanceux lorsque le nombre de lancers fut entre 2 et 6, et qu'il est extrêmement malchanceux de l'avoir obtenu en plus de 6 lancers.
...
Béatrice et Martine s'entraînent au lancer du poids. Au cours des dernières années, elles ont dû s'affronter 10 fois dans des compétitions. Pour Béatrice, son meilleur lancer (en mètres) pour chacune des compétitions est donné par le vecteur B. Les meilleurs lancers de Martine se trouvent dans le vecteur M.
...
Ainsi, pour accéder au résultat de Béatrice lors de sa deuxième compétition, il suffit de noter l'indexation 2:
...
On aimerait écrire le pseudo-code nous permettant de déterminer qui de Béatrice ou de Martine a obtenu le meilleur lancer pour chacune des compétitions.

<div style='text-align:center;'>
<img src="assets/img/poids.PNG" height="400px"></img>
</div>
...
```
IF B[1] < M[1]
  PRINT "Martine a gagné la compétition 1"
IF ELSE B[1] > M[1]
  PRINT "Béatrice a gagné la compétition 1"
ELSE
...
```
IF B[2] < M[2]
  PRINT "Martine a gagné la compétition 2"
IF ELSE B[2] > M[2]
  PRINT "Béatrice a gagné la compétition 2"
ELSE
...
FOR NumCompet in 1:10
  IF B[NumCompet] < M[NumCompet]
    PRINT ("Martine a gagné la compétition" NumCompet)
  IF ELSE B[NumCompet] > M[NumCompet]
    PRINT ("Béatrice a gagné la compétition" NumCompet)
  ELSE
...
*Pour ordonner des lettres, il faut savoir que $A=1$, $B=2$, ... , $Z = 26$. 

Ainsi, $B<A =$ FALSE, car $2\nless1$.
