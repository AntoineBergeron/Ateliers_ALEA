...
Dans cet exemple, notre séquence contient 3 éléments "Bonjour", "programmeurs", "en R". R va donc évaluer l'expression 3 fois (3 éléments). La variable index, ici `etape`, prendra successivement les valeurs de l'élément 1 à 3 pendant l'exécution de la boucle.
...
tempC <- -50:100 
tempF <- tempC*1.8 + 32 
resultat <- matrix(c(tempC, tempF), ncol=2) 
}
```
...
*Pour vérifier votre réponse, il est possible d'utiliser la fonction `factorial()`
...
La boucle de type *while* répète une série d'instructions tant qu'une condition est respectée. C'est une version générale de la boucle *for*. Par exemple, on peut tirer deux pièces de monnaie jusqu'à ce que l'on obtienne la combinaison pile-pile. 

Rappel: (!=) signifie non-égale

```{r}
piece <- c("pile","face")
combinaison = "face-face"
while(combinaison != "pile-pile") {
    essai1 <- sample(piece, 1)  # OU sample(piece, 2, replace = TRUE)
...
n_steps <- 115
N <- numeric(n_steps+1)
N[1] <- 220 # Année 0 en position 1 (donc [position - 1] = nombre d'années écoulées)
...
.pull-left[
<div style='text-align:center;'>
<img src="assets/img/ampoule.png" width="80%"></img>
</div>
]
...
xlab = "Nombre d'années avant de griller")
```
]
...
```{r, out.width = '45%', dpi=300, fig.align="center"}
...
X <- matrix(runif(3000*3000),nr=3000,nc=3000)
```
...
.pull-right[
<div style='text-align:center;'>
<img src="assets/img/animation1.gif" height="500px"></img>
</div>
]
...
.pull-left[
<img src="assets/img/typedarbre.png" height="160px"></img>

<div style='text-align:center;'>
<img src="assets/img/arbre.png" height="200px"></img>
</div>
]


.pull-right[
<img src="assets/img/defarbres.png" height="350px"></img>
]